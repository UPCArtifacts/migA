...
package fr.uphf.se.kotlinresearch.diff.heuristic.mtdiff.intern;
...
import fr.uphf.se.kotlinresearch.diff.heuristic.mtdiff.MtDiffOptimizedMatcher;
import fr.uphf.se.kotlinresearch.diff.heuristic.mtdiff.hungarian.DoubleMatrix;
import fr.uphf.se.kotlinresearch.diff.heuristic.mtdiff.hungarian.Hungarian;
import fr.uphf.se.kotlinresearch.diff.heuristic.mtdiff.similarity.InnerNodeSimilarityCalculator;
import fr.uphf.se.kotlinresearch.diff.heuristic.mtdiff.similarity.NGramCalculator;
...
class SimilarityMatrixHelper {
	...
	SimilarityMatrixHelper(boolean[][] aggregationFinished, ITree[] firstAggregations, ITree[] secondAggregations,
			ConcurrentHashMap<ITree, MatchingCandidate> currentResultMap, AtomicBoolean changed,
			ArrayList<ITree> oldNodes, ArrayList<ITree> newNodes, IdentityHashMap<ITree, Mapping> resultMap,
			NGramCalculator stringSim, ConcurrentHashMap<String, Float> stringSimCache, boolean onlyOneClassPair,
	...
	/**
	 * Creates a NodeAggregation that consists of the specified node, it's parent
	...
	private float simpleSimilarity(final ITree tree1, final ITree tree2,
		...
		InnerNodeSimilarityCalculator nodeMatcher = new InnerNodeSimilarityCalculator(labelConfiguration, leavesMap1,
				leavesMap2, directChildrenMap1, directChildrenMap2, currentResultSet);
			...
			ConcurrentHashMap<ITree, ConcurrentHashMap<ITree, Float>> similarityCache, AtomicLong similarityEntries) {
		{
			...
			for (int j = 0; j < newNodes.size(); j++) {
				if (!aggregationFinished[ipar][j]) {
					if (firstAggregations[ipar] != null && firstAggregations[ipar].getType() != NodeAggregation.TAG) {
