 ...
 */
package fr.uphf.se.kotlinresearch.diff.heuristic.mtdiff;
...
import fr.uphf.se.kotlinresearch.diff.heuristic.mtdiff.intern.InnerMatcherMtDiffRunnable;
import fr.uphf.se.kotlinresearch.diff.heuristic.mtdiff.intern.LMatcher;
import fr.uphf.se.kotlinresearch.diff.heuristic.mtdiff.intern.LabelConfiguration;
import fr.uphf.se.kotlinresearch.diff.heuristic.mtdiff.intern.LeafSimResults;
import fr.uphf.se.kotlinresearch.diff.heuristic.mtdiff.intern.LeafSimilarityRunnable;
import fr.uphf.se.kotlinresearch.diff.heuristic.mtdiff.intern.MatchingCandidate;
import fr.uphf.se.kotlinresearch.diff.heuristic.mtdiff.intern.PairComparator;
import fr.uphf.se.kotlinresearch.diff.heuristic.mtdiff.intern.PostOrderSetGenerator;
import fr.uphf.se.kotlinresearch.diff.heuristic.mtdiff.intern.SimilarLeafExaminationRunnable;
import fr.uphf.se.kotlinresearch.diff.heuristic.mtdiff.intern.TagComparator;
import fr.uphf.se.kotlinresearch.diff.heuristic.mtdiff.intern.TreeMatcherConfiguration;
import fr.uphf.se.kotlinresearch.diff.heuristic.mtdiff.similarity.InnerNodeSimilarityCalculator;
...
public class MtDiffOptimizedMatcher extends Matcher {
	...
	private void computeMatchingPairs() throws Exception {
		...
		final IdentityHashMap<ITree, ArrayList<ITree>> directChildrenMap1 = setGeneratorNew.getDirectChildrenMap();
		...
		final IdentityHashMap<ITree, ArrayList<ITree>> directChildrenMap2 = setGeneratorNew.getDirectChildrenMap();
	...
	private void matchInnerNodes(IdentityHashMap<ITree, Integer> orderedList1,
		...
		InnerMatcherMtDiffRunnable runnable = new InnerMatcherMtDiffRunnable(unmatchedNodesOrdered1,
				unmatchedNodesOrdered2, innerMatchCounter, start, unmatchedNodesOrdered1.size(), innerMatcher2);
		if (executorService != null) {
			results.add(executorService.submit(runnable));
		} else {
			FutureTask<ArrayList<MatchingCandidate>> task = new FutureTask<>(runnable);
	...
	private void matchLeaves(IdentityHashMap<ITree, Integer> orderedList1, IdentityHashMap<ITree, Integer> orderedList2,
			...
			ArrayList<ITree> leaves1tmp, ArrayList<ITree> leaves2tmp, HashSet<ITree> skipList, LMatcher leafMatcher,
		...
		for (Entry<Integer, Integer> tagEntry : list) {
			...
			while (start + step < nodes.length) {
				LeafSimilarityRunnable runnable = new LeafSimilarityRunnable(stringSimCache, nodes, subLeaves2,
						leafSimCounter, start, start + step, orderedList1, orderedList2, skipList, leafMatcher,
			...
			LeafSimilarityRunnable runnable = new LeafSimilarityRunnable(stringSimCache, nodes, subLeaves2,
					leafSimCounter, start, nodes.length, orderedList1, orderedList2, skipList, leafMatcher,
					labelConfiguration, renames);
			if (executorService != null) {
				leafSimResults.add(executorService.submit(runnable));
			} else {
			...
			for (Future<LeafSimResults<ITree>> fu : leafSimResults) {
				...
				try {
					...
					for (final Entry<ITree, ArrayList<MatchingCandidate>> entry : leafSimRes.subleafCandidateMap
							.entrySet()) {
						ArrayList<MatchingCandidate> myList = leafCandidateMap.putIfAbsent(entry.getKey(),
			...
			for (final Entry<ITree, ArrayList<MatchingCandidate>> entry : leafCandidateMap.entrySet()) {
			...
			Entry<ITree, ArrayList<MatchingCandidate>>[] array = (Entry<ITree, ArrayList<MatchingCandidate>>[]) tieCandidates
					.entrySet().toArray(new Entry[tieCandidates.size()]);
			...
			for (int i = 0; i < oldNodeArray.size(); i++) {
				...
				SimilarLeafExaminationRunnable treeDiffRunnable = new SimilarLeafExaminationRunnable(oldNodeList,
						...
						leavesMap2, labelConfiguration, leafMatcher, directChildrenMap1, directChildrenMap2, getSrc(),
