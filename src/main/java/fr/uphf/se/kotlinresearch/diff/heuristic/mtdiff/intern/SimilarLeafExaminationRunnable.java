...
public class SimilarLeafExaminationRunnable implements Callable<Set<MatchingCandidate>> {
	...
	public SimilarLeafExaminationRunnable(ArrayList<ITree> oldNodes, ArrayList<ITree> newNodes,
			...
			Map<ITree, ArrayList<ITree>> leavesMap2, LabelConfiguration labelConfiguration, LMatcher leafMatcher,
	...
	public Set<MatchingCandidate> call() throws Exception {
		try {
			...
			final ConcurrentSkipListSet<MatchingCandidate> resultList = new ConcurrentSkipListSet<>(
					new PairComparator(orderedListOld, orderedListNew));
			final ConcurrentHashMap<ITree, MatchingCandidate> currentResultMap = new ConcurrentHashMap<>();
			...
			for (MatchingCandidate mc : initialList) {
				...
				if (tmp == null) {
					...
					ConcurrentHashMap<ITree, MatchingCandidate> tmp2 = candidateMap.putIfAbsent(mc.first, tmp);
					if (tmp2 != null) {
						tmp = tmp2;
					}
				}
			...
			new SimilarityMatrixHelper(aggregationFinished, firstAggregations, secondAggregations, currentResultMap,
					changed, oldNodes, newNodes, resultMap, stringSim, stringSimCache, onlyOneClassPair,
					similarityScores, initialList, candidateMap, foundMaxArray, similarityCache, similarityEntries,
					parents1, parents2, leavesMap1, leavesMap2, labelConfiguration, leafMatcher, directChildrenMap1,
